"""Reconstruct criteria-study packets, decisions and sealed outcomes without model calls."""
...
from jev_atari.study import sample_training
...
def verify_probe(root, states, programs):
    ...
    for row in rows:
        ...
        request = p.request(obs, "jev-1.13.0")
        ...
        assert prediction["requested_model"] == prediction["response_model"] == "jev-1.13.0"
    report = read_json(root / "results.json")
    assert report["attempts"] == len(exchanges) <= 520
    ...
    if report["status"] == "complete":
        ...
        item = order[len(rows)]
        request = programs[item["program"]].request(lookup[item["id"]]["observation"], "jev-1.13.0")
...
def verify(root, out, repository):
    ...
    for path in sorted(root.rglob("plan.json")):
        ...
        if value.get("kind") not in {"pong-study-episode-plan-v1", KIND + "-final-episode"}:
            continue
    ...
    for search in range(3):
        ...
        for round_index in range(2):
            ...
            for arm in ("B", "A"):
                ...
                if arm == "A":
                    evidence = sample_training(
                        [
                            rd / "training" / f"seed-{s}" / "jev" / f"seed-{s}"
                            for s in TRAIN[search][round_index]
                        ]
                    )
    ...
    usage_events = []
    for execution_path in root.glob("search-*/round-*/[AB]/teacher/invocation-*/execution.json"):
    ...
    assert teacher_count == budget["teacher_invocations"] <= 16
    ...
    costs = {
        ...
        "billing": "Not recorded; no currency claim.",
    }
